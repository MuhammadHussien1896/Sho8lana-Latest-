    ...
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessage(string message,string receiverId,int serviceId)
        {
            ...
            if(receiverConnectionIds != null)
            {
                ...
                {
                    await Clients.Client(receiverConnectionId).SendAsync("ReceiveMessage", message, senderId, senderName, date,serviceMessage.MessageId);
                    
                }
            }
            await Clients.Client(Context.ConnectionId).SendAsync("ReceiveMessage", message, senderId,senderName, date, serviceMessage.MessageId);
